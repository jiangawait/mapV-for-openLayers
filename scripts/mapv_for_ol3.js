(function(global, factory) {
	...
	(function(exports) {
			...
			draw: function draw(context, dataSet, options) {
				...
				if (options.bigData) {
					...
					if (type == 'Point' || type == 'Polygon' || type == 'MultiPolygon') {
						...
						if (item && options && (item.strokeStyle || options.strokeStyle) && options.lineWidth) {
							context.stroke();
						}
		...
		*/
		 ...
		 * @author Jiang Lingbin
		 * @date 2017/03/24
		 */
		function CanvasLayer$3(options) {
			this.options = options || {};
			this.paneName = this.options.paneName || 'mapPane';
			this.context = this.options.context || '2d';
		...
		if (global$2.ol) {
			...
			// goog.inherits(CanvasLayer$3, ol.MyOverlay);
			// CanvasLayer$3.prototype = new ol.MyOverlay();

			CanvasLayer$3.prototype.initialize = function(map) {
				// this._map = map;
				var that = this;
				var canvas = that.canvas = document.createElement("canvas");
				canvas.style.cssText = "position:absolute;" + "left:0;" + "top:0;" + "z-index:" + that.zIndex + ";user-select:none;";
				canvas.style.mixBlendMode = that.mixBlendMode;
				canvas.className = "mapvClass";
				that.adjustSize();
				// map.getPanes()[that.paneName].appendChild(canvas);
				// map.getViewport().insertBefore(canvas, map.getViewport().childNodes[0]);

				// goog.events.listen(map, ol.MapEventType.POSTRENDER, function() {
				//     // that.adjustSize();
				//     that.draw();
				// }, true, that);
				that.postcomposeKey = map.on('postcompose', function(event) {
					// that.adjustSize();
					that.draw();
				});

				that.changeSizeKey = map.on('change:size', function(event) {
					that.adjustSize();
					// that.draw();
				});
				// return that.canvas;
			...
			CanvasLayer$3.prototype.adjustSize = function() {
				var size = this._map.getSize();
				// var canvas = this.canvas;

				var devicePixelRatio = this.devicePixelRatio = global$2.devicePixelRatio;

				// this.canvas.width = size[0] * devicePixelRatio;
				// this.canvas.height = size[1] * devicePixelRatio;
			...
			CanvasLayer$3.prototype.draw = function() {
				var self = this;
				clearTimeout(self.timeoutID);
				self.timeoutID = setTimeout(function() {
					self._draw();
			...
			CanvasLayer$3.prototype._draw = function() {
				// var map = this._map;
				var size = this._map.getSize();
				var center = this._map.getView().getCenter();
				if (center) {
					// var pixel = this._map.pointToOverlayPixel(center);
			...
			CanvasLayer$3.prototype.getContainer = function() {
				return this.canvas;
			};

			CanvasLayer$3.prototype.show = function() {
				// if (!this.canvas) {
				//     this._map.addOverlay(this);
				// }
				this.canvas.style.display = "block";
				this._map.getViewport().insertBefore(this.canvas, this._map.getViewport().childNodes[0]);
			};

			CanvasLayer$3.prototype.hide = function() {
				this.canvas.style.display = "none";
				this._map.getViewport().removeChild(this.canvas);
				this._map.unByKey(this.postcomposeKey);
				this._map.unByKey(this.changeSizeKey);

				// this.options = {};
				// delete this.canvas;
				// delete this._map;
				// delete this._lastDrawTime;
				var me = this;
				$.each(me, function(index, object) {
					delete me[index];
				});
				//this._map.removeOverlay(this);
			};

			CanvasLayer$3.prototype.setZIndex = function(zIndex) {
				this.canvas.style.zIndex = zIndex;
			};

			CanvasLayer$3.prototype.getZIndex = function() {
				return this.zIndex;
			};
		}

		function Layer$3(map, dataSet, options) {
			if (!(dataSet instanceof DataSet)) {
				dataSet = new DataSet(dataSet);
			}
			...
			var canvasLayer = this.canvasLayer = new CanvasLayer$3({
				map: map,
				context: this.context,
				paneName: options.paneName,
				mixBlendMode: options.mixBlendMode,
		...
		Layer$3.prototype.clickEvent = function(e) {
			var pixel = e.pixel;
			var context = this.canvasLayer.canvas.getContext(this.context);
			var data = this.dataSet.get();
			for (var i = 0; i < data.length; i++) {
		...
		Layer$3.prototype.mousemoveEvent = function(e) {
			var pixel = e.pixel;
			var context = this.canvasLayer.canvas.getContext(this.context);
			var data = this.dataSet.get();
			for (var i = 0; i < data.length; i++) {
		...
		Layer$3.prototype.bindEvent = function(e) {
			var map = this.map;
			...
			if (this.options.methods) {
				...
				if (this.options.methods.mousemove) {
					map.addEventListener('pointermove', this.mousemoveEvent);
				}
			}
		};

		Layer$3.prototype.unbindEvent = function(e) {
			var map = this.map;
		...
		// 经纬度左边转换为墨卡托坐标
		Layer$3.prototype.transferToMercator = function() {
			var me = this;
			var projection = me.map.getView().getProjection();

			if (me.options.coordType !== 'bd09mc') {
				var data = me.dataSet.get();
				data = me.dataSet.transferCoordinate(data, function(coordinates) {
					// var pixel = me.map.getPixelFromCoordinate(coordinates);
					//var pixel = ol.proj.fromLonLat(coordinates);
				...
				me.dataSet._set(data);
			}
			me = null;
		};

		Layer$3.prototype._canvasUpdate = function(time) {
			if (!this.canvasLayer) {
				return;
			}
			...
			var map = self.map; //self.canvasLayer._map;

			// var zoomUnit = Math.pow(2, 18 - map.getView().getZoom());
			// var projection = map.getView().getProjection();
			// var size = map.getSize();
			// var extent = map.getView().calculateExtent(size);
			// var mcCenter = map.getView().getCenter(); //ol.proj.fromLonLat(map.getView().getCenter()); // projection.lngLatToPoint(map.getCenter());
			...
			var dataGetOptions = {
				fromColumn: 'coordinates', // self.options.coordType == 'bd09mc' ? 'coordinates' : 'coordinates_mercator',
				transferCoordinate: function transferCoordinate(coordinate) {
					// var pixel = map.getPixelFromCoordinate(coordinate);
					// var dx = size[0]*(coordinate[0] - extent[0])/(extent[2]-extent[0]);
					// var dy = size[1]*(coordinate[1] - extent[1])/(extent[3]-extent[1]);

					// var PosX = dx;// + size[0] / 2;
					// var PosY = size[1] - dy;// + size[1] / 2;
					if (self.options.coordType == 'bd09mc')
			...
			if (self.options.minZoom && map.getView().getZoom() < self.options.minZoom || self.options.maxZoom && map.getView().getZoom() > self.options.maxZoom) {
				return;
			}
			...
			switch (self.options.draw) {
				...
				case 'honeycomb':
					...
					var nwPixel = map.pointToPixel(new BMap.Point(minx, maxy));
					*/
					var nwPixel = map.getPixelFromCoordinate([0, 0]);
					self.options.offset = {
						x: nwPixel.x,
						y: nwPixel.y
					};
		...
		Layer$3.prototype.isEnabledTime = function() {
		...
		Layer$3.prototype.argCheck = function(options) {
			if (options.draw == 'heatmap') {
				if (options.strokeStyle) {
					console.warn('[heatmap] options.strokeStyle is discard, pleause use options.strength [eg: options.strength = 0.1]');
				}
		...
		Layer$3.prototype.init = function(options) {
			var self = this;
			...
			if (self.options.draw == 'time' || self.isEnabledTime()) {
				...
				self.listenKey = self.map.on('moveend', function() {
					if (self.isEnabledTime() && self.animator) {
						steps.step = animationOptions.stepsRange.start;
						self.animator.stop();
		...
		Layer$3.prototype.show = function() {
			// this.map.addOverlay(this.canvasLayer);

			this.canvasLayer && this.canvasLayer.show();
		};

		Layer$3.prototype.hide = function() {
			// this.map.removeOverlay(this.canvasLayer);
			var self = this;
			clear(self.canvasLayer.canvas.getContext(self.context));
			self.animator && self.animator.stop();
			// self.animator = null;
			if (self.listenKey)
				self.map.unByKey(self.listenKey);
			// self.map.un('moveend',function(){});
			self.canvasLayer && self.canvasLayer.hide();
			self.dataSet.clear();
			$.each(self, function(index, object) {
				delete self[index];
			});
		};

		Layer$3.prototype.destroy = function() {
			this.unbindEvent();
			this.hide();
		};
		 ...
		 * obj.options
		 */
		Layer$3.prototype.update = function(obj) {
			var self = this;
			var _options = obj.options;
			var options = self.options;
			for (var i in _options) {
		...
		Layer$3.prototype.setOptions = function(options) {
			var self = this;
			self.init(options);
			self.canvasLayer.draw();
		};

		Layer$3.prototype.set = function(obj) {
			var conf = {
				globalAlpha: 1,
				globalCompositeOperation: 'source-over',
				imageSmoothingEnabled: true,
		...
		Layer$3.prototype.getLegend = function(options) {
			var draw = this.options.draw;
			var legend = null;
			if (self.options.draw == 'intensity' || self.options.draw == 'heatmap') {
				return this.intensity.getLegend(options);
		...
		exports.csv = csv;
		exports.olMapCanvasLayer = CanvasLayer$3;
		exports.olMapLayer = Layer$3;
